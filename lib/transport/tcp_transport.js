...
import { writeTCPMessageHeader } from "lib/nodeopcua";
import { readRawMessageHeader } from "lib/misc/message_builder_base";
 ...
 */
class TCP_transport extends EventEmitter {
    constructor(...args) {
        super(...args);
      /**
       * timeout
  ...
  /**
   * ```createChunk``` is used to construct a pre-allocated chunk 
   * to store up to ```length``` bytes of data.
   * The created chunk includes a prepended header for 
   *   ```chunk_type``` of size ```self.headerSize```.
   *
   * @method createChunk
   * @param msg_type
   ...
   *  - a created chunk should be committed using the ```write``` method 
   *    before an other one is created.
   */
    createChunk(msg_type, chunk_type, length) {
        assert(msg_type === "MSG");
   ...
   */
    _install_socket(socket) {
      ...
      // install packet assembler ...
        ...
        self.packetAssembler.on("message", (message_chunk) => {
   ...
   *
   * Rules:
   * * TCP_transport will not emit the ```message``` event, 
   *    while the "one time message receiver" is in operation.
   * * the TCP_transport will wait for the next complete message chunk 
   *    and call the provided callback func
   *   ```callback(null,messageChunk);```
   * * if a messageChunk is not received within ```TCP_transport.timeout``` 
   *    or if the underlying socket reports an error,
...
function _on_socket_ended_message(err) {
    ...
    const _err = err || new Error("_socket has been disconnected by third party");

    self.on_socket_ended(_err);

    self.__disconnecting__ = true;

    debugLog(" bytesRead    = ", self.bytesRead);
    debugLog(" bytesWritten = ", self.bytesWritten);
    _fulfill_pending_promises.call(self, new Error(`Connection aborted - ended by server : ${_err ? _err.message : ""}`));
}
