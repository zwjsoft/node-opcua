...
import OPCUAClientBase from "./OPCUAClientBase";
import StatusCodes from "../datamodel/opcua_status_code";
import { 
 ...
 * @constructor
 */
class OPCUAClient extends OPCUAClientBase {
    constructor(options = {}) {
        super(options);
    ...
    // get applicationURI from certificate
        ...
        if (certificate) {
            ...
            applicationUri = makeApplicationUrn(get_fully_qualified_domain_name(), this.applicationName);
        }
        return applicationUri;
    }
    __resolveEndPoint() {
        this.securityPolicy = this.securityPolicy || SecurityPolicy.None;
        let endpoint = this.findEndpoint(this._secureChannel.endpoint_url, this.securityMode, this.securityPolicy);
    ...
    _createSession(callback) {
        ...
        self.__createSession_step2(session, callback);
        return undefined;
    }
    ...
    // a client Nonce must be provided if security mode is set
        ...
        // xx console.log("xxxxx response",response.responseHeader.serviceResult);
                if (response.responseHeader.serviceResult === StatusCodes.BadTooManySessions) {
                    myErr = new Error(`Too Many Sessions : ${response.responseHeader.serviceResult.toString()}`);
          ...
          // todo: verify SignedSoftwareCertificates and  response.serverSignature
                    ...
                    const mySession = session || new ClientSession(self);
                    mySession.name = request.sessionName;
                    mySession.sessionId = response.sessionId;
                    mySession.authenticationToken = response.authenticationToken;
                    mySession.timeout = response.revisedSessionTimeout;
                    mySession.serverNonce = response.serverNonce;
                    mySession.serverCertificate = response.serverCertificate;
                    mySession.serverSignature = response.serverSignature;

                    debugLog("revised session timeout = ".yellow, session.timeout);

                    self._server_endpoints = response.serverEndpoints;
                    mySession.serverEndpoints = response.serverEndpoints;
                } else {
                    myErr = new Error(`??? ${response.responseHeader.serviceResult}`);
                }
            }
            if (myErr) {
                callback(myErr);
  ...
  // see OPCUA Part 4 - $7.35
    ...
    // make sure session is attached to this client
        ...
        self.createUserIdentityToken(session, self.userIdentityInfo, (err, userIdentityToken) => {
            if (err) {
        ...
        // The SignatureAlgorithm depends on the identity token type.
            ...
            session.performMessageTransaction(request, (err, response) => {
                if (!err && response.responseHeader.serviceResult === StatusCodes.Good) {
                    ...
                    session._client = _old_client;
                    callback(err || new Error(response.responseHeader.serviceResult.toString()), null);
                }
            });
        });
    }
   ...
   *
   */
    createSession(userIdentityInfo, callback) {
        const self = this;
        ...
        self._createSession((err, session) => {
            if (err) {
                callback(err);
            } else {
                self._addSession(session);
                self._activateSession(session, err => callback(err, session));
            }
        });
    }

    changeSessionIdentity(session, userIdentityInfo, callback) {
        const self = this;
        assert(_.isFunction(callback));
        ...
        self._activateSession(session, err =>  callback(err));
    }
   ...
   */
    ...
    // todo : send close session on secure channel
        self._closeSession(session, deleteSubscriptions, (err) => {
    ...
    _ask_for_subscription_republish(session, callback) {
    ...
    // call base class implementation first
        OPCUAClientBase.prototype._on_connection_reestablished.call(self, (err) => {
      //
      ...
      // repair session
            ...
            async.map(sessions, (session, next) => {
                ...
                self._activateSession(session, (err) => {
          //
          ...
          //
            ...
            //   if failed => recreate a new Channel and transfer the subscription
                ...
                // create new session
                                self.__createSession_step2(session, (err, _new_session) => {
                            ...
                            (callback) => {
                                ...
                                self._activateSession(new_session, (err) => {
                ...
                // Transfer subscriptions
                                ...
                                new_session.transferSubscriptions(options, (err, results) => {
                                    if (err) {
                                        return callback(err);
                                    }
              ...
              //      call Republish
                            callback => self._ask_for_subscription_republish(new_session, callback)
                        ], next);
                    } else {
            //      call Republish
    ...
    createUserIdentityToken(session, userIdentityToken, callback) {
        assert(_.isFunction(callback));
        const self = this;
 ...
 * @param callback {function}
 */
    withSession(endpointUrl, inner_func, callback) {
      ...
      // close session
            (callback) => {
                the_session.close(/* deleteSubscriptions=*/true, (err) => {
                    if (err) {
...
function createUserNameIdentityToken(session, userName, password) {
  ...
  // if server does not provide certificate use unencrypted password
    if (serverCertificate == null) {
        const userIdentityToken = new UserNameIdentityToken({
            userName,
