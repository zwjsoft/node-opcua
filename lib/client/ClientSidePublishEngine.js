   ...
   */
    ...
    _send_publish_request() {
      ...
      // in our case:
        ...
        self.session.publish(publish_request, (err, response) => {
            self.nbPendingPublishRequests -= 1;
            if (err) {
      ...
      // Publish responses
        function _republish(subscription,subscriptionId,_i_callback) {
            ...
            function _send_republish(_b_callback) {
                ...
                self.session.republish(request,(err, response) => {
            ...
            setImmediate(() => {
                ...
                async.whilst(() => !done,_send_republish,(err) => {
        ...
        function repairSubscription(subscription,subscriptionId,_the_callback) {
            _republish(subscription,subscriptionId,(err) => {
...
export default ClientSidePublishEngine;
