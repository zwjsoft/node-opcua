...
import { TimestampsToReturn } from "lib/services/read_service";
import ClientSession from "lib/client/ClientSession";
import ClientMonitoredItem from "lib/client/ClientMonitoredItem";
 ...
 */
class ClientSubscription extends EventEmitter {
    constructor(session, options) {
      ...
      // ];
        ...
        options.maxNotificationsPerPublish  = isNullOrUndefined(options.maxNotificationsPerPublish) ? 0  :  options.maxNotificationsPerPublish;
       ...
       */
        ...
        setImmediate(() => {
            self.__create_subscription((err) => {
                if (!err) {
                    setImmediate(() => {
                      /**
   ...
   * @param done
   */
    monitorItems(itemsToCreate, timestampsToReturn, done) {
    ...
    setPublishingMode(publishingEnabled, callback) {
        assert(_.isFunction(callback));
        const self = this;
        self.session.setPublishingMode(publishingEnabled, self.subscriptionId, (err, results) => {
            if (err) {
                return callback(err);
            }
        ...
        async.series([
            ...
            (callback) => {
              ...
              // re-create monitored items
                ...
                const itemsToCreate = [];
                _.forEach(monitoredItems_old,(monitoredItem, clientHandle) => {
                ...
                subscription.session.createMonitoredItems(createMonitorItemsRequest, (err, response) => {
                    if (!err) {
