...
function dumpStateMachineToPlantUML(stateMachineType) {
    ...
    stateMachineType.states.forEach((state) => {
        w(` ${s(state)}: ${n(state)}`);
    });
...
function dumpStateMachineToGraphViz(/* UAStateMachineProxy*/ stateMachineType) {
    ...
    stateMachineType.transitions.forEach((transition) => {
        w(`  _${s(transition.fromStateNode)} -> _${s(transition.toStateNode)} [ , label = "${n(transition)}" ];`);
    });

    w("}");
}

export { dumpStateMachineToPlantUML, dumpStateMachineToGraphViz };
