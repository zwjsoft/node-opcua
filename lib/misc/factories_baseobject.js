...
import { getBaseSchema } from "lib/misc/factories_schema_helpers";
...
class BaseUAObject {
    ...
    toString(...args) {
        ...
        if (!self.explore) {
            //xx console.log(require("util").inspect(self));
        ...
        schema.fields.forEach((field) => {
            ...
            if (field.isArray) {
                ...
                for (let i = 0; i < nb; i++) {
                    ...
                    _decodeMember(value, field, stream, options);
    ...
    explore(...args) {
        ...
        applyOnAllSchemaFields(self, self._schema, data, _exploreObject, args);
        data.lines.push("};");
    ...
    toJSON(...args) {
        ...
        const options = {};
        _visitSchemaChain(self, schema, options, _JSONify);
        return options;
    }
    ...
    clone() /* options,optionalfilter,extraInfo*/ {
        ...
        function constructParam(schema, options) {
        ...
        constructParam.call(this, self._schema, params);
...
function _decodeMember(value, field, stream, options) {
    const tracer = options.tracer;
...
function applyOnAllSchemaFields(self, schema, data, callback, args) {
    assert(schema);
...
function _arrayEllypsis(value, field) {
    if (!value) {
        return "null []";
    }
    if (value.length === 0) {
...
const _exploreObject = (self, field, data, args) => {
    ...
    const _dump_simple_value = (self, field, data, value, fieldType) => {
        ...
        if (value instanceof Buffer) {
            ...
            if (field.isArray) {
                str = `${fieldName_f} ${fieldType_f}: ${_arrayEllypsis(value, field)}`;
            } else {
                ...
                if (field.isArray) {
                    if (value === null) {
                        ...
                        for (i = 0; i < m; i++) {
                            ...
                            applyOnAllSchemaFields(element, _new_desc, data1, _exploreObject, args);
                    ...
                    applyOnAllSchemaFields(value, _new_desc, data1, _exploreObject, args);
                    data.lines = data.lines.concat(data1.lines);
...
function _visitSchemaChain(self, schema, options, func, extraData) {
    ...
    func.call(self, schema, options, extraData);
}
