...
import { StatusCodes } from "lib/raw_status_codes";
...
const decodeStatusCode = (stream) => {
    ...
    if (!sc) {
        throw new Error(`expecting a known StatusCode 0x${code_without_info_bits.toString(16)}`);
    }
    if (info_bits) {
...
export { decodeStatusCode };

/* construct status codes fast search indexes */
let StatusCodes_reverse_map = {};
_.forEach(StatusCodes, (code) => {
...
ModifiableStatusCode.prototype._getExtraName = function () {
    const self = this;
    const str = [];
    _.forEach(extraStatusCodeBits,(value, key) => {
...
exports.StatusCodes.makeStatusCode = (statusCode, optionalBits) => {
